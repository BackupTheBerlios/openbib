...
#  diskmab2meta.pl
#
#  Konvertierung von MAB2-Daten aus dem Disketten- in das Meta-Format
#
...
if (!$filename){
    print << "HELP";
diskmab2meta.pl - Aufrufsyntax

    diskmab2meta.pl --filename=xxx
HELP
exit;
}
...
my $mexdefs_ref = {
    ...
    '012'  => {           # identifikationsnummer des zugehoerigen Titelsatzes
        newcat => '0004', # Zugeordn. Titelsatz
        mut    => 1,
    },
    '100'  => {           # Signatur
...
my $processed_tit_ref   = {};
my $processed_mex_ref   = {};
...
my $multcount_tit_ref = {};
my $multcount_mex_ref = {};
my $ignore_line = 0;
    ...
    if (/^\#\#\#........................([a-z])/){
        ...
        if ($ignore_line){
            ...
            if ($category =~ /^001$/ || $category =~ /^010$/){
                $content=~s/\D//g;
                if ($category =~ /^001$/){
                    if ($processed_tit_ref->{$content}){
                        $ignore_line=1;
                        next;
                    }
                    $processed_tit_ref->{$content}=1;
                }
            }
            ...
            if ($newcategory =~ /^0662$/){
                ...
                foreach my $part (@multcontent){
                   if ($part =~/^u(.+)$/){
                      $content=$1;
                   }
#                   if ($part =~/^zInhaltsverzeichnis/){
#                      $newcategory=4110;
#                   }
                }
            }
            ...
            if ($newcategory && $titdefs_ref->{$category}{mult} && $content){
                my $multcount=sprintf "%03d",++$multcount_tit_ref->{$newcategory};
            ...
            if ($category =~ /^001$/ || $category =~ /^010$/){
                ...
                if ($category =~ /^001$/){
                    if ($processed_mex_ref->{$content}){
                        next;
                    }
                    $processed_mex_ref->{$content}=1;
                }
            ...
            if ($newcategory && $mexdefs_ref->{$category}{mult} && $content){
                my $multcount=sprintf "%03d",++$multcount_mex_ref->{$newcategory};
